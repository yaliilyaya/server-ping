 ...
 */
final class Version20220708120932 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('DROP INDEX IDX_27A63B1BE04EA9');
        $this->addSql('CREATE TEMPORARY TABLE __temp__service_job_report AS SELECT id, job_id, result, status FROM service_job_report');
        $this->addSql('DROP TABLE service_job_report');
        $this->addSql('CREATE TABLE service_job_report (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, job_id INTEGER DEFAULT NULL, result VARCHAR(255) NOT NULL COLLATE BINARY, status VARCHAR(255) NOT NULL COLLATE BINARY, command VARCHAR(255) NOT NULL, CONSTRAINT FK_27A63B1BE04EA9 FOREIGN KEY (job_id) REFERENCES service_job (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO service_job_report (id, job_id, result, status) SELECT id, job_id, result, status FROM __temp__service_job_report');
        $this->addSql('DROP TABLE __temp__service_job_report');
